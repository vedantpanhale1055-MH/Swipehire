...
//
// Runs in small concurrent chunks (not all at once, not fully sequential) —
// firing every job at Groq simultaneously was hitting free-tier rate limits,
// which silently failed some jobs and left their cards with no match score.
// Each job also gets one retry before being given up on.
const CONCURRENCY = 4;
...
async function scoreOneWithRetry(resumeText, job) {
  try {
    ...
    // Likely a transient rate-limit or network blip — retry once.
    await sleep(500);
    try {
      const fit = await scoreJobFit(resumeText, job.description);
      return { ...job, fit };
    } catch (err2) {
      return { ...job, fit: null, error: err2.message };
    }
  }
}
