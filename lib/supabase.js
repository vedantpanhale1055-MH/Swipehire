...
// Sets/updates the interview date+time for a saved job (used by the
// Calendar page). Pass an ISO string, or null to clear a scheduled date.
export async function updateInterviewDate(jobId, isoDateTime) {
  const { data, error } = await supabase
    .from('saved_jobs')
    .update({ interview_date: isoDateTime })
    .eq('id', jobId)
    .select();
  if (error) throw error;
  return data[0];
...
// Persists a tailored-resume result onto the job row (used by the
// "Save to Documents" action in TailoredResumeModal).
export async function saveTailoredResume(jobId, result) {
  const { data, error } = await supabase
    .from('saved_jobs')
    .update({ tailored_resume: result, tailored_resume_at: new Date().toISOString() })
    .eq('id', jobId)
    .select();
  if (error) throw error;
  return data[0];
...
// Minimal bar for "good enough to match/tailor against" — a name and at
// least one skill. Used to decide whether to route a user through
// /onboarding before letting them into the app.
...
// Fetches a profile for the public portfolio page. Reads from the
// public_profiles view (see portfolio-migration.sql), which only exposes
// rows where is_public = true and never includes phone/email — so this is
// safe to call for anonymous, logged-out visitors.
export async function getPublicProfileByUsername(username) {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) throw error;
  return data; // null if no public profile exists at this username
}

// Checks whether a username is free (case-insensitive), excluding the
// current user's own row so re-saving your own username doesn't collide
// with yourself.
